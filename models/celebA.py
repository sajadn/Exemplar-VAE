...
class VAE(AbsModel):
    ...
    def create_model(self, args, train_data_size=None):
        ...
        self.q_z_mean = nn.Conv2d(in_channels=512, out_channels=8, kernel_size=5, stride=1, padding=2)
        self.q_z_logvar = nn.Conv2d(in_channels=512, out_channels=8, kernel_size=5, stride=1, padding=2)

        coeff = 2 if args.less_upsample else 1
        self.p_x_layers = nn.Sequential(
            #nn.Upsample(scale_factor=2),
            nn.Conv2d(in_channels=8, out_channels=512, kernel_size=5, stride=1, padding=2),
            nn.BatchNorm2d(512),
            nn.ELU(),
            nn.Upsample(scale_factor=2),
