...
class VAE(AbsModel):
    ...
    def create_model(self, args, train_data_size=None):
        ...
        self.q_z_layers = nn.Sequential(
            weight_norm(nn.Conv2d(in_channels=self.args.input_size[0], out_channels=128, kernel_size=5, stride=2, padding=2)),
            nn.ELU(),
        ...
        if self.args.input_type == 'gray' or self.args.input_type == 'continuous':
            self.p_x_mean = weight_norm(nn.Conv2d(in_channels=128, out_channels=self.args.input_size[0], kernel_size=5, stride=1, padding=2))
